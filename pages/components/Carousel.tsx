...
export default function Carousel({
  ...
  const [displayedList, setDisplayedList] = useState([products[first]]);
  ...
  useEffect(() => {
    if (window.matchMedia("(min-width: 1366px)").matches) {
      setDesktopView(true);
      setDisplayedList([products[first], products[second]]);
    }
  });
    ...
    <React.Fragment>
      ...
      <Stack direction="horizontal">
        ...
        <img
          ...
          onClick={() => {
            if (desktopView) {
              first === products.length - 1 ? setFirst(0) : setFirst(first + 1);
