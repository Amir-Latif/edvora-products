...
const Home: NextPage<{ productList: product[] }> = ({ productList }) => {
  ...
  function filter(item: string) {
    ...
    setFilteredList(Array.from(filteredList));
  }
  ...
  return (
    <React.Fragment>
      ...
      <main className={`${Styles.container} container`}>
        <div className={`${Styles.itemsContainer} ${Styles.filtersContainer}`}>
          ...
          >
            {filteredList.length > 0 &&
              filteredList.map((element) => (
                <Col key={element} className={`${Styles.item} ${Styles.dropdownItem}`}>
                  {element}
                </Col>
              ))}
          </Row>
